...
if __name__ == '__main__':
    ...
    for txt in valid_txt:
        digit = int(txt[:-4])
        #shutil.copyfile(os.path.join(train_path, "%d.ann" % digit), "valid/%d.ann" % digit)
        shutil.copyfile(os.path.join(train_path, txt), "valid/%s" % txt)
        context = open(os.path.join(train_path, "%d.txt" % digit), "r", encoding="UTF-8").read()
        ...
        标签增强部分==>将没有完全标注的都标注上
        """
        labels = set({s['label'] for s in src})
        for label in labels:
            all_match = re.finditer(pattern=re.compile(label), string=context)
            for x in all_match:
                any_overlap = False
                for e in src:
                    if not (e["start"] >= x.end() or e["end"] <= x.start()):
                        any_overlap = True
                        break
                if not any_overlap:
                    new_obj = {"type": q_type, "start": x.start(), "end": x.end(), "label": label}
                    src.append(new_obj)

        with open(os.path.join("valid/%d.ann" % digit), "w", encoding="UTF-8") as wfile:
            for i, item in enumerate(src):
                wfile.write(f"T{i+1}\t{item['type']} {item['start']} {item['end']}\t{item['label']}\n")
...
#    with open("../user_data/tmp_data/process.pkl", "wb") as f:
