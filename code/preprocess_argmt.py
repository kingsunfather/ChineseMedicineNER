...
if __name__ == '__main__':
    ...
    for i, txt in enumerate(train_txt):
        ...
        for key in output["query_map"].keys():
            ...
            if i < len(train_txt) // 2:
                output["train_items_1"].extend(q_c_pairs)
            else:
                output["train_items_2"].extend(q_c_pairs)
    
    for i in range(3):
        shuffle(output["train_items_1"])
        shuffle(output["train_items_2"])
    ...
    with open("../user_data/tmp_data/process_argmt.pkl", "wb") as f:
