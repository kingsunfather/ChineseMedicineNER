    ...
    for txt in test_txt:
        ...
        for key in output["query_map"].keys():
            ...
            for item in q_c_pairs:
                item["BIE_label"] = get_multihot_label(vocab, item["context"])
            t["query"].extend(q_c_pairs)
    return test_items


def build_vocab(path, save_to='vocab.txt'):
    vocab = set()
    anns = [i for i in os.listdir(train_path) if ".ann" in i]
    for fname in anns:
        ann_items = open(os.path.join(path, "%s" % fname), "r", encoding="UTF-8").readlines()
...
def get_multihot_label(vocab, text):
    """得到text每个字符可能的位置BMES，并进行multihot编码，4代表开头，5代表记载开头出现过也在结尾出现过，以此类推"""
    label = np.zeros((len(text), 4), dtype=np.int)
    for w in vocab:
        s = 0
        while True:
            s = text.find(w, s)
            ...
            if len(w) == 1:
                ...
                label[s, 0] = 1
                label[s+1:s+len(w)-1, 1] = 1
                label[s+len(w)-1, 2] = 1
            s += len(w)
    #for w in vocab:
    #    all_match = re.finditer(pattern=re.compile(w), string=text)
    #    for match in all_match:
    #        label[match.start(), 0] = 1
    #        label[match.start()+1:match.end()-1, 1] = 1
    #        label[match.end()-1, 2] = 1
    label = label[:, 0] * 8 + label[:, 1] * 4 + label[:, 2] * 2 + label[:, 3]
...
if __name__ == '__main__':
    ...
    test_path = "../data/chusai_xuanshou"
    vocab = build_vocab(train_path)
    train_txt = [i for i in os.listdir(train_path) if ".txt" in i]
    seed(0)
    shuffle(train_txt)
    num_valid = int(len(train_txt) * 0.1)
    train_txt, valid_txt = train_txt[num_valid:], train_txt[:num_valid]
    test_txt = [i for i in os.listdir(test_path)]
    for i, txt in enumerate(train_txt):
        if i % 10 == 0:
            print(i)
        digit = int(txt[:-4])
        context = open(os.path.join(train_path, "%s" % txt), "r", encoding="UTF-8").readline()
        ann = open(os.path.join(train_path, "%d.ann" % digit), "r", encoding="UTF-8").readlines()
        src = []
        for item in ann:
            t = re.split("\s", item)
            ...
            标签增强部分==>将没有完全标注的都标注上
            """
            #all_match = re.finditer(pattern=re.compile(label), string=context)
            #for x in all_match:
            #    new_obj = {"type": q_type, "start": x.start(), "end": x.end() - 1, "label": label}
            #    append_flag = True
            #    for y in src:
            #        if y == new_obj:
            #            append_flag = False
    ...
    for txt in valid_txt:
        ...
        for item in ann:
            ...
            #all_match = re.finditer(pattern=re.compile(label), string=context)
            #for x in all_match:
            #    new_obj = {"type": q_type, "ans_s": x.start(), "ans_e": x.end(), "label": label}
            #    append_flag = True
            #    for y in t["answer"]:
            #        if y == new_obj:
            #            append_flag = False
    ...
    #for txt in test_txt:
    #    digit = int(txt[:-4])
    #    context = open(os.path.join(test_path, "%s" % txt), "r", encoding="UTF-8").readline()
    #    t = {"id": str(digit), "query": []}
    ...
    with open("../user_data/tmp_data/process_sw.pkl", "wb") as f:
