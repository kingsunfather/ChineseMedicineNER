...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bie_label = torch.tensor([0] * (len(query_tokens) + 2) + bie_label + [0] + [0] * extra)
        _bie_label = torch.zeros(self.max_enc_len, 4)
        for i in range(4):
            _bie_label[:, i] = bie_label // 2**(3-i)
            bie_label = bie_label % 2**(3-i)
...
class EvalDataset(object):
    ...
    def __getitem__(self, index):
        ...
        out = {
            ...
            "context_list": context_list, "bie_list": torch.chunk(torch.stack(bie_list, dim=0), chunks=chunk_num, dim=0)
        }
        if self.is_test:
            out["id"] = x["id"]
        else:
...
class MyModel(nn.Module):
    ...
    def __init__(self, pre_train_dir: str, dropout_rate: float):
        ...
        self.roberta_encoder = BertModel(config)
        self.encoder_linear = torch.nn.Sequential(
            torch.nn.Linear(in_features=1024 + 4, out_features=1024),
            torch.nn.Tanh(),
    ...
    def forward(self, input_ids, input_mask, input_seg, bie_label, start_label=None, end_label=None, seq_mask=None):
        ...
        encoder_rep = torch.cat((encoder_rep, bie_label), dim=-1)
...
class Main(object):
    def __init__(self, train_loader, valid_loader, test_flag=False, test_loader=None):
        ...
        # 内部词典
        self.vocab = {line.strip() for line in open('inner_vocab.txt', encoding="UTF-8").readlines()}
        self.vocab = sorted(self.vocab, key=len, reverse=True)
        # 外部词典
        self.kw2cls = json.load(open("ext_vocab.json", encoding="UTF-8"))
        if "" in self.kw2cls:
            del self.kw2cls[""]
    ...
    def train(self):
        ...
        while True:
            for item in self.train_loader:
                input_ids, input_mask, input_seg, seq_mask, start_label, end_label, bie_label = \
                    item["input_ids"], item["input_mask"], item["input_seg"], item["seq_mask"], \
                    item["start_label"], item["end_label"], item["bie_label"]
                ...
                loss = self.model(
                    ...
                    bie_label=bie_label, start_label=start_label.to(device),
            ...
            for item in self.valid_loader:
                ...
                for i in range(len(input_ids_chunks)):
                    s_seq, e_seq = self.model(
                        ...
                        input_seg=input_seg_chunks[i].to(device), bie_label=bie_list[i].to(device)
                    )
    ...
    def eval_models(self):
        model_path = glob.glob("/home/ldmc/quanlin/project/user_data/model_data/best_score*.pth")
        models = []
        for path in model_path:
            model = MyModel(pre_train_dir=args["pre_train_dir"], dropout_rate=args["dropout"])
            model.load_state_dict(torch.load(path, map_location=device), strict=True)
            model = model.to(device)
            models.append(model.eval())
        print("#model:", len(models))
        threshs = [0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85]
        print("#thresh:", len(threshs))
        y_pred, y_true = [[] for i in range(len(threshs))], []
        with torch.no_grad():
            for item in self.valid_loader:
                input_ids_chunks, input_seg_chunks, input_mask_chunks, type_list = \
                    item["input_ids_chunks"], item["input_seg_chunks"], item["input_mask_chunks"], item["type_list"]
                ...
                for i in range(len(input_ids_chunks)):
                    ...
                    for model in models:
                        ...
                        s_seqs.append(s_seq)
                        e_seqs.append(e_seq)
                    s_seq = torch.mean(torch.stack(s_seqs), 0)
                    e_seq = torch.mean(torch.stack(e_seqs), 0)
                    s_seq_list.extend(s_seq.cpu().numpy())
                    e_seq_list.extend(e_seq.cpu().numpy())
                for j, th in enumerate(threshs):
                    ...
                    #tmp = self.post_process(item["id"], context_list[i], tmp)
                    y_pred[j].append({"answer": tmp})
        self.model.train()
        for i, th in enumerate(threshs):
            p, r, f1 = self.calculate_strict_f1(y_pred=y_pred[i], y_true=y_true)
            print(f"threshold: {th}, f1: {f1}, p: {p}, r: {r}.")
    ...
    def test(self):
        ...
        with torch.no_grad():
            for item in self.test_loader:
                ...
                tmp = self.post_process(text_id, context_list[i], tmp)
    ...
    def test_models(self):
        model_path = glob.glob("../user_data/model_data/best_score*.pth")
        models = []
        for path in model_path:
            model = MyModel(pre_train_dir=args["pre_train_dir"], dropout_rate=args["dropout"])
            model.load_state_dict(torch.load(args["save_path"], map_location=device), strict=False)
            model = model.to(device)
            models.append(model.eval())
    ...
    def post_process(self, tid, context, out):
        # 后处理：使用外部词典查漏补缺
        labels = [e["label"] for e in out]
        out_kws = sorted(set(self.kw2cls.keys()) - set(labels), key=len, reverse=True)
        for kw in out_kws:
            p = 0
            if len(kw) < 4:
                continue
            while True:
                ...
                for e in out:
        	    # 判断是否有重合交叉等情况
                    if not (e["start"] >= p+len(kw) or e["end"] <= p):
                        any_overlap = True
                        break
        		#print(f"{tid}.ann: {e}, {kw}")
        	# 没有重合或交叉则添加该实体
                if not any_overlap:
                    out.append({"type": self.kw2cls[kw], "start": p, "end": p+len(kw), "label": kw})
                    assert context[p:p+len(kw)] == kw
                    #print(f"{tid}.ann find new entity {kw} at ({p},{p+len(kw)}).")
                p += len(kw)
        return out

    def sequence_dec(self, s_seq, e_seq, q_type, start, end, c_start, distance, context):
        ans_index = []
        i_start, i_end = [], []
        ...
        for item in ans_index:
            ...
            if "，" in label:
                p = label.find("，")
                out.append({"type": q_type, "start": distance + item[0] - c_start, "end": distance + item[0] - c_start + p,
                            "label": label[:p]})
                for kw in self.vocab:
                    if kw in label[p+1:]:
                        p = label.find(kw, p+1)
                        out.append({"type": q_type, "start": distance + item[0] - c_start + p, "end": distance + item[0] - c_start + p + len(kw),
                                    "label": label[p:p+len(kw)]})
                        break
            else:
            ...
            for item in p_answer:
                for jtem in t_answer:
                    if item["type"] == jtem["type"] and item["start"] == jtem["ans_s"] and item["end"] == jtem["ans_e"]:
                        exact_match += 1
        precision, recall = exact_match / (sum_pred + 1e-5), exact_match / (sum_true + 1e-5)
...
if __name__ == "__main__":
    ...
    args = {
        ...
        "pre_train_dir": "../user_data/model_data",
        "submit_dir": "../result",
        "clip_norm": 0.25,
        "dimension": 1024,
        "max_enc_len": 512,
    ...
    with open("../user_data/tmp_data/process_sw.pkl", "rb") as f:
        x = pickle.load(f)
    ...
    if sys.argv[1] == "train":
        ...
        #test_items = get_test_items('../../tcdata/juesai')
        test_loader = EvalDataset(data=test_items, max_enc_len=args["max_enc_len"], is_test=True)
